...
def _autocorrelation_plot(
    param, samples, fig=None, color=color, markersize=0.5, grid=True
):
...
def _autocorrelation_plot_mcmc(
    param, samples, colorcycle=colorcycle, grid=True
):
    """Generate the autocorrelation function for a set of samples for a given
...
def _sample_evolution_plot(
    param, samples, latex_label, inj_value=None, fig=None, color=color,
...
def _sample_evolution_plot_mcmc(
    param, samples, latex_label, inj_value=None, colorcycle=colorcycle,
    grid=True
):
...
def _1d_cdf_plot(
    param, samples, latex_label, fig=None, color=color, title=True,
...
def _1d_cdf_plot_mcmc(
    param, samples, latex_label, colorcycle=colorcycle, grid=True, **kwargs
):
...
def _1d_analytic_plot(
    ...
    title=True, color=color, autoscale=True, grid=True, set_labels=True,
    plot_percentile=True, xlims=None, label=None, linestyle="-",
    linewidth=1.75, injection_color=injection_color,
...
def _1d_histogram_plot(
    ...
    prior=None, weights=None, fig=None, ax=None, title=True, color=color,
    ...
    linestyle="-", injection_color=injection_color, _default_hist_kwargs={
    ...
    if len(set(samples)) <= max_vline:
        ...
        if hist:
            ...
            if prior is not None:
                ...
                _ = ax.hist(
                    prior, color=prior_color, alpha=0.2, edgecolor="w",
        ...
        if kde:
            ...
            if prior is not None:
                kdeplot(
                    prior, color=prior_color, ax=ax, linestyle=linestyle,
                    **kwargs
                )
...
def _1d_histogram_plot_mcmc(
    param, samples, latex_label, colorcycle=colorcycle, **kwargs
):
...
def _1d_histogram_plot_bootstrap(
    param, samples, latex_label, colorcycle=colorcycle, nsamples=1000,
...
def _make_corner_plot(
    ...
    default_kwargs = corner_kwargs.copy()
...
def _make_comparison_corner_plot(
    samples, latex_labels, corner_parameters=None, colors=corner_colors,
